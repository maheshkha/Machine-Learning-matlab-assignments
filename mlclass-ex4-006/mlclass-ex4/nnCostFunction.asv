...
n1 = [ones(m, 1) X]; % add 1
z2 = n1 * Theta1';
n2 = [ones(size(sigmoid(z2), 1), 1) sigmoid(z2)]; % add 1
z3 = n2 * Theta2';
...
J = 1/m * sum(sum(-1 * y_recoded .* log(a3)-(1-y_recoded) .* log(1-a3))) + regularator;
...
for t = 1:m
	...
	% Big delta update
	Theta1_grad = Theta1_grad + delta_2 * n1';
	Theta2_grad = Theta2_grad + delta_3 * n2';
end
